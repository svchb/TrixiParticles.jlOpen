...
k       = argmin(d2)                       # index of nearest existing node
cx, cy  = coords[1, k]+0.1*dx, coords[2, k]-0.1*dx      # shift slightly to avoid weird alignment issues

coords_w = reshape([cx, cy], 2, 1)   # 2×1 matrix
vel_w    = zeros(2, 1)               # 2×1 matrix (matches)

water_ic = InitialCondition(
    coordinates      = coords_w,
    velocity         = vel_w,
    density          = ρ_water,
    particle_spacing = dx,          # needed for set operations / auto mass
)
...
bd_density = fill(ρ_water, length(tank_air.boundary.density))
bd_mass    = fill(ρ_water * dx^2, length(tank_air.boundary.mass))
...
save_cb     = SolutionSavingCallback(dt=0.02, prefix="single_particle_fall_switched")
