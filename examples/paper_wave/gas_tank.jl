...
const water_density  = 1.0        # [kg/m³] (sea water)
...
const nu_empirical=max(0.001*Δx_f, air_viscosity)
...
const tvf_pressure_air = 1.0 * sound_speed^2 * air_density # 0.1 * 360 * 1.2 = 43.2
...
# =============================================================================
# ==== 2.  Geometry & tank
initial_water_size = (tank_length, tank_height)
tank_size          = (tank_length, tank_height)

@info "Δx = $(Δx_f*1000) mm  |  tank = $(tank_size) m  |  fluid = $(initial_water_size) m"

tank = RectangularTank(Δx_f, initial_water_size, tank_size, 1000.0;
                       n_layers = boundary_layers, spacing_ratio = 1,
                       acceleration = (0.0, -gravity), state_equation = air_eos)
...
save_cb     = SolutionSavingCallback(; dt = 0.025, output_directory = "output/coastal_wave_spray_2d", prefix = "water_gas_tvf_modconti_c$(Int(sound_speed))_r$(resolution_factor)_bg0-2")
# stepsize_cb = StepsizeCallback(cfl = 0.9)
# cbset       = CallbackSet(info_cb, save_cb, stepsize_cb, post_cb)
cbset       = CallbackSet(info_cb, save_cb, post_cb)
